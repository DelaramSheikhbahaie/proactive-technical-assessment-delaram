...
<template>
  <div
    class="w-full bg-white p-4 my-2 flex flex-col items-start justify-between rounded-lg shadow-md hover:shadow-lg transition-shadow">
    <NuxtImg :src="character.image || DEFAULT_AVATAR" alt="Character avatar"
      class="w-full mb-4 rounded-lg object-cover" />
    <p class="text-lg w-full font-semibold mb-4 truncate">{{ character.name }}</p>
